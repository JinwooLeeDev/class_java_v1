...
public class FileServer {
    ...
    // 서버 - 클라이언트 약속 (프로토콜)
    // [1 바이트] 파일의 이름의 길이 (0~255) -> [1][1][1][1][1][1][1][1] (8비트)
    // [N 바이트] 파일 이름 (실제 데이터)
    ...
    public static void main(String[] args) {
        ...
        // 그냥 uploads 라는 경로를 가리키는 객체가 메모리에 생성됐을 뿐이다.

        File dir = new File(UPLOAD_DIR);            // 파일이 저장 될 경로를 입력해둔 dir 선언

        if (!dir.exists()) {
            // mkdirs() 을 호출하면 실제 폴더가 생성됨
        ...
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {  // 서버소켓 생성하고
            Socket socket = serverSocket.accept();  // 연결까지 블로킹상태

            System.out.println("클라이언트 연결 됨 " + socket.getInetAddress().getHostAddress());   //

            //소켓에 연결된 입출력 스트림 준비
            InputStream in = socket.getInputStream();       // 인풋 스트림 in
            OutputStream out = socket.getOutputStream();    // 아웃풋 스트림 out 각 각 선언
            ...
            // byte는 기본적으로 -128 ~ +127 까지가 범위라 "끝(-1)"을 표현할 자리가 없다.
            // 그래서 0 ~ 255 는 데이터를 구분해서 -1이 들어오면 끝 신호로 쓴다. ( read() )
            int nameLength = in.read();         // 소켓을 통해 들어온 스트림 1바이트씩 읽어서 정수 형태로 nameLength에 저장

            // 방어적 코드
            // 이 검사를 빼면 아래에서는 new byte[-1]이 되어 오류가 발생한다.
            if (nameLength == -1) {     // 만약 소켓을 통해 들어온게 -1 (없다면) 아래 코드 실행, 방어적 코드
                System.out.println("클라이언트가 아무것도 보내지 않고 종료 했습니다.");
                return;
            }
            System.out.println("파일의 이름 길이 : " + nameLength + "바이트");

            // ------------------------------------------------
            // 2단계 : 파일 이름을 정확하게 nameLength 만큼 읽기
            // ------------------------------------------------
            byte[] nameBuffer = new byte[nameLength];       // 정수 형태로 저장된 nameLength만큼의 길이를 가진 바이트 형태의 nameBuffer 배열 선언에 저장
            int nameRead = 0;   // 지금까지 읽어서 채운 바이트 수
            // 이 while 문법이 이번 예제의 가장 중요한 부분이라 할 수 있다.
            // read(값을 담을 배열, 시작위치, 최대개수)는 요청한 만큼 반드시 읽어준다는 보장이 없다.
            // 네트워크 사정에 따라 총 30바이트를 받는다고 가정하면 첫 번째에 3바이트만 올 수도 있다.
            while (nameRead < nameLength) {     // 읽어온 바이트가 들어온 내용보다 작다면 다시 읽어오기 반복을 위한 while문
            ...
            File target = new File(dir, fileName);  // dir는 경로, fileName은 말 그대로 어떤 이름을 저장할지

            // FileOutputStream 생성자가 실행되는 순간 비로서 디스크에 실제 파일이 생긴다. (있으면 내용이 지워지고 새로 시작)
